...
Meteor.methods({
    ...
    removeSpark: function (spark){
        check(spark, String);
        var removeSpark = Sparks.findOne(spark);
        var bulb = Bulbs.findOne(removeSpark.bulbId);
        if(!removeSpark)
            throw new Meteor.Error('invalid', 'Spark not found');

        var upvotes = removeSpark.upvotes;
        var downvotes = removeSpark.downvotes;
        ...
        while(comment = Comments.findOne({sparkId: removeSpark._id})){
