...
Meteor.methods({
    projectInsert: function(projectAttributes) {
        ...
        var projectWithSameTitle = Projects.findOne({name: projectAttributes.name});
        if (projectWithSameTitle && (projectWithSameTitle._id != projectAttributes._id)) {
            return {
                projectExists: true,
    ...
    projectEdit: function(projectAttributes) {
        ...
        if (errors.name || errors.url)
            throw new Meteor.Error('invalid-project', "You must set a name and description for your project");
        ...
        Projects.update({_id: projectAttributes._id}, {
            $set: {
    ...
    projectInviteUser: function(projectId, projectUsers, userAttributes){
        ...
        if (_.contains(user.pendingProjects, projectId)) {
